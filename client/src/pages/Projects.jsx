      ...
      <div className="flex gap-2 p-1.5 bg-gray-900 rounded-2xl border border-gray-800 w-fit">
        {['All', 'Active', 'Completed'].map(status => (
          <button
            ...
            className={`px-6 py-2.5 rounded-xl text-sm font-bold transition-all ${filter === status
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {loading ? (
          ...
          filteredProjects.map((project) => (
            <div key={project._id} className="bg-gray-900/50 rounded-[40px] p-8 border border-gray-800 hover:border-gray-700 transition-all shadow-sm group">
              <div className="flex justify-between items-start mb-6">
                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${project.status === 'Completed'
              ...
              <p className="text-gray-500 text-sm mb-8 font-medium">Client: <span className="text-gray-300">{project.clientId?.name || 'N/A'}</span></p>

              <div className="grid grid-cols-2 gap-4 py-6 border-t border-gray-800/50">
                <div>
                  <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Budget</p>
                  <p className="text-lg font-bold text-white">${project.budget?.toLocaleString()}</p>
                </div>
                <div>
                  <p className="text-[10px] text-gray-500 uppercase font-black tracking-widest mb-1">Deadline</p>
