    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-400 mt-1">Hello, <span className="text-blue-400 font-bold">{user?.name}</span>. Your operational metrics are synced.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {loading ? (
          ...
          stats.map((stat) => (
            <div key={stat.label} className="bg-gray-900 p-8 rounded-[32px] border border-gray-800 shadow-sm hover:border-gray-700 transition-all group">
              <p className="text-[10px] text-gray-500 mb-1 font-black uppercase tracking-[0.2em]">{stat.label}</p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-gray-900 rounded-[40px] border border-gray-800 p-10 shadow-sm">
            ...
            <div className="h-[300px] flex items-center justify-center">
              <Pie data={projectChartData} options={{ 
                maintainAspectRatio: false, 
                plugins: { legend: { position: 'bottom', labels: { color: '#9ca3af', font: { weight: '600' }, usePointStyle: true, padding: 20 } } } 
              }} />
            </div>
          </div>
          ...
          <div className="bg-gray-900 rounded-[40px] border border-gray-800 p-10 shadow-sm">
            ...
            <div className="h-[300px]">
              <Bar data={taskChartData} options={{ 
                maintainAspectRatio: false, 
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-6 p-6 bg-gray-900 rounded-3xl border border-gray-800/50">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">All cloud assets and client records are encrypted and accessible.</p>
              </div>
            </div>
          </div>
        </div>
