...
function ProjectDetails() {
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      const token = localStorage.getItem('token')
      await axios.put(`${BASE_API_URL}/api/tasks/${draggableId}`, 
  ...
  return (
    <div className="space-y-10 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-gray-800">
        <div>
          <Link to="/projects" className="text-sm text-blue-400 hover:underline flex items-center gap-1 mb-4">
          ...
          <p className="text-gray-400 mt-2">Budget: <span className="text-white font-bold">${project.budget?.toLocaleString()}</span></p>
        </div>
        <button
            ...
            <div key={column} className="flex flex-col h-full">
              <div className="flex items-center justify-between mb-4 px-2">
                <div className="flex items-center gap-2">
                  <h3 className="text-sm font-bold uppercase tracking-widest text-gray-500">{column}</h3>
                ...
                {(provided, snapshot) => (
                  <div 
                    ...
                    className={`flex-1 rounded-3xl border border-gray-800/50 p-4 space-y-4 min-h-[400px] transition-colors ${
                      snapshot.isDraggingOver ? 'bg-blue-500/5 border-blue-500/20' : 'bg-gray-900/30'
                    }`}
                  >
                        ...
                        {(provided, snapshot) => (
                          <div 
                            ...
                            className={`bg-gray-900 p-5 rounded-2xl border border-gray-800 shadow-sm hover:border-gray-700 transition-all group ${
                              snapshot.isDragging ? 'shadow-2xl border-blue-500/50 scale-[1.02] z-50 ring-1 ring-blue-500/50' : ''
                            }`}
                          >
                            <div className="flex justify-between items-start mb-4">
                              ...
                              <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => handleEditTask(task)} className="p-1 text-gray-500 hover:text-white">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
                                  </svg>
                                </button>
                                <button onClick={() => handleDeleteTask(task._id)} className="p-1 text-gray-500 hover:text-red-400">
                                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="bg-gray-900 rounded-3xl border border-gray-800 p-8 shadow-sm h-fit">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-gray-200">{new Date(project.createdAt).toLocaleDateString()}</p>
            </div>
            <div>
              <p className="text-xs text-gray-500 uppercase font-bold tracking-wider mb-1">Last Updated</p>
              <p className="text-sm text-gray-200">{new Date(project.updatedAt).toLocaleDateString()}</p>
            </div>
          </div>
        </div>
      </div>
